  ...
  async getData() {
    try {
      const endpointDatabase =
        "https://raw.githubusercontent.com/htquanq/database/main/banana.json";
      const { data } = await axios.get(endpointDatabase);
      return data;
    } catch (error) {
      console.log(colors.red("Lấy dữ liệu server htquanq thất bại"));
      return null;
    }
  }
      ...
      console.log(
        colors.yellow(
          `🚀 Đã có phiên bản mới ${colors.blue(
            ...
            "https://github.com/htquanq/banana"
          )}`
        )
      );
      console.log("");
